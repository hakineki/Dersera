...
export async function oturumBilgisi(): Promise<{ hesap: HesapOzeti | null; davetGerekli: boolean } | undefined> {
  try {
    const res = await fetch("/api/auth/ben", { cache: "no-store" });
    if (!res.ok) return undefined;
    const json = await res.json();
    return { hesap: json.hesap ?? null, davetGerekli: !!json.davetGerekli };
  } catch {
    return undefined;
  }
}
