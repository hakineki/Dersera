...
// Marka zeminli ikon karesi (rengi kullanan yer verir): panel sekmeleri mor, sayfa dışı bağlantılar yeşil, çıkış kırmızı.
export const IKON_KUTUSU = "w-8 h-8 shrink-0 rounded-lg flex items-center justify-center";
        ...
        <>
          ...
          >
            {sekmeler.map((s) => (
              ...
              >
                <span className={`${IKON_KUTUSU} ${aktif === s.id ? "bg-indigo-600 text-white" : "bg-indigo-50 text-indigo-700"}`}>
                  <Ikon ad={s.icon} />
                </span>
                <span>{s.label}</span>
              </button>
            ...
            >
              <span className={`${IKON_KUTUSU} bg-red-50 text-red-700`}>
                <Ikon ad="cikis" />
              </span>
              <span>Çıkış yap</span>
            </button>
